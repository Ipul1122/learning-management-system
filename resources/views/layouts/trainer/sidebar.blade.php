...
<aside class="hidden lg:flex lg:flex-col w-64 bg-[#1E1B18] text-white flex-shrink-0 border-r border-[#322E2B] transition-all duration-300">
    ...
    <div class="flex-1 py-6 px-4 space-y-1.5 overflow-y-auto font-quicksand">
        ...
        <!-- Verifikasi Kelulusan 20 JP (Fase 5) -->
        @php
            $pendingGraduationsCount = \App\Models\GraduationSubmission::where('trainer_id', Auth::id())->where('status', 'pending')->count();
        @endphp
        <a href="{{ route('trainer.graduations.index') }}"
           class="flex items-center justify-between px-3.5 py-2.5 rounded-xl font-montserrat font-bold text-xs transition duration-150 {{ request()->routeIs('trainer.graduations.*') ? 'bg-gradient-to-r from-[#FF6B00] to-[#E11D48] text-white shadow-md' : 'text-[#FAF8F5]/80 hover:text-white hover:bg-white/5' }}">
            ...
            @if($pendingGraduationsCount > 0)
                <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-500 text-slate-950 font-mono">
...
<div x-show="sidebarOpen"
    ...
    <div x-show="sidebarOpen"
        ...
        <div class="flex-1 py-6 px-4 space-y-1.5 overflow-y-auto">
            ...
            <a href="{{ route('trainer.graduations.index') }}"
               class="flex items-center justify-between px-3.5 py-2.5 rounded-xl font-montserrat font-bold text-xs {{ request()->routeIs('trainer.graduations.*') ? 'bg-gradient-to-r from-[#FF6B00] to-[#E11D48] text-white' : 'text-white/80 hover:bg-white/5' }}">
                <div class="flex items-center gap-3">
                    <svg class="w-5 h-5 fill-none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                    </svg>
                ...
                @if(isset($pendingGraduationsCount) && $pendingGraduationsCount > 0)
                    <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-500 text-slate-950 font-mono">
                        {{ $pendingGraduationsCount }}
                    </span>
                @endif
