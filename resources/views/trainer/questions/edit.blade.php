<x-app-layout>
    <div class="py-8" x-data="questionEditManager()">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div>
                ...
                <h1 class="font-montserrat font-extrabold text-2xl sm:text-3xl text-[#1E1B18] tracking-tight">
                    Edit Butir Soal #{{ $question->id }}
                </h1>
                <p class="font-quicksand text-xs text-[#6E675F] mt-1">
                    Perbarui teks pertanyaan, pilihan opsi jawaban, atau pembahasan soal di cabang <strong class="text-[#1E1B18]">{{ $branch->name }}</strong>.
                </p>
            </div>

            <!-- Form Card -->
            <div class="bg-white rounded-3xl border border-[#EBE5DF] shadow-sm overflow-hidden">
                <form method="POST" action="{{ route('trainer.questions.update', $question) }}" class="p-6 sm:p-8 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea id="question_text"
                                  ...
                                  rows="4"
                                  required
                                  class="w-full px-4 py-3 rounded-2xl border border-[#EBE5DF] bg-[#FAF8F5] text-sm text-[#1E1B18] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#FF6B00] font-quicksand transition">{{ old('question_text', $question->question_text) }}</textarea>
                        @error('question_text')
                    ...
                    <div x-show="questionType !== 'essay'" class="space-y-3">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-[11px] text-[#6E675F]">Pilih tombol radio pada opsi yang merupakan kunci jawaban yang benar.</p>
                            </div>
                    ...
                    <div>
                        ...
                        <textarea id="explanation"
                                  name="explanation"
                                  rows="3"
                                  class="w-full px-4 py-3 rounded-2xl border border-[#EBE5DF] bg-[#FAF8F5] text-sm text-[#1E1B18] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#FF6B00] font-quicksand transition">{{ old('explanation', $question->explanation) }}</textarea>
                        @error('explanation')
                            <p class="text-xs text-[#DC2626] mt-1 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    @push('scripts')
        <script>
            function questionEditManager() {
                const initialOptions = @json($question->options->map(fn($opt) => [
                ...
                return {
                    ...
                    handleTypeChange(type) {
                        ...
                        } else if (type === 'multiple_choice' && this.options.length < 2) {
                            this.options = [
                                { option_text: '', is_correct: true },
                                { option_text: '', is_correct: false }
                            ];
