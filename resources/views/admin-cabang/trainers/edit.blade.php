        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div>
                ...
                <p class="font-quicksand text-sm text-[#6E675F] mt-1">
                    Perbarui profil instruktur pengajar di <strong>{{ $branch->name }}</strong>.
                </p>
            </div>

            <!-- Form Card -->
            <div class="bg-white rounded-2xl border border-[#EBE5DF] shadow-sm p-6 sm:p-8">
                <form method="POST" action="{{ route('cabang.trainers.update', $trainer) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="p-4 rounded-xl bg-[#FAF8F5] border border-[#EBE5DF] space-y-4">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-montserrat font-bold text-xs uppercase tracking-wider text-[#1E1B18]">
                                Reset Kata Sandi Trainer (Kosongkan jika tidak diubah)
                            </span>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="status" value="active" {{ old('status', $trainer->status) === 'active' ? 'checked' : '' }} class="text-[#FF6B00] focus:ring-[#FF6B00]">
                                <span class="text-sm font-quicksand text-[#1E1B18]">Aktif</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="status" value="inactive" {{ old('status', $trainer->status) === 'inactive' ? 'checked' : '' }} class="text-[#FF6B00] focus:ring-[#FF6B00]">
